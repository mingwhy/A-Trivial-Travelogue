...
foreach my $file (@sorted_files)
	{ next if (($file eq '.') or ($file eq '..'));
	  $files_seen++;
	  print "file index: $files_seen of $files_total\n";
	  my $bioproject_id = '';
	  ...
	  while(<IN>)
		{ my $line = $_; $line=~s/\s+$//; #chomp($line);
		  my @line = split(/\t/,$line); #print "@line\n";		  
		  if ($. == 1)
			{ #print "@line\n";
			  for(my $x=0;$x<@line;$x++)
				{ my $val = $line[$x]; #get interested column index frist 1st row (colnames)
				  if 	($val eq 'sample_accession')  	  { $sample_id_num		   = $x; print 'sample_id_num ',$sample_id_num,"\n"}
				  elsif ($val eq 'run_accession')     	  { $run_num 	           = $x; print 'run_num ',$run_num,"\n"}
				  ...
				  elsif ($val eq 'library_strategy')  	  { $library_strategy_num  = $x; print 'library_strategy ',$library_strategy_num,"\n"}
				  elsif ($val eq 'library_source')	  	  { $library_source_num    = $x; }
				  elsif ($val eq 'library_layout')    	  { $library_layout_num    = $x; }				  
				  elsif ($val eq 'instrument_model_desc') { $platform_num 		   = $x; print 'instrument_model_desc ',$platform_num,"\n"}				  
				  elsif ($val eq 'organism_taxid')	  	  { $taxid_num   		   = $x; print "taxid_num $taxid_num\n"} # NOTE: the space in 'organism_taxid ' is NOT an error but is necessary to parse these files
				  ...
				  elsif ($val eq 'ncbi_url')         	  { $download_path3_num    = $x; print 'ncbi_url ',$download_path3_num,"\n"}
	  ...
	  close(IN) or die $!;
	  #print "check $sample_id_num, $run_num, $spot_num \n";
	  #print "check $library_strategy_num $library_source_num, $library_layout_num, $platform_num \n";
	  #print "check $taxid_num $download_path0_num $download_path1_num $download_path2_num $download_path3_num\n";
