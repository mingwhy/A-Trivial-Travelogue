...
########################################################
dim(ifly$S) #8135 11898
mz.adj.mat=matrix(0,nrow=nrow(ifly$S),ncol=nrow(ifly$S))
for(i in 1:ncol(ifly$S)){
  one.rxn=ifly$S[,i]
  left.mz<-which(one.rxn==1)
  right.mz<-which(one.rxn==-1)
  links=as.data.frame(expand.grid(left.mz,right.mz))
  for(x in 1:nrow(links)){mz.adj.mat[links[x,1],links[x,2]]=1}
}
sum(mz.adj.mat) #27565
rownames(mz.adj.mat)=colnames(mz.adj.mat)=ifly$mets
library(igraph)
g1=graph_from_adjacency_matrix(mz.adj.mat,mode = "undirected")
...
head(share.mz)
node1=which(mets.mapping$metKEGGID=='C02835')
node2=which(mets.mapping$metKEGGID=='C00430')

# too slow, graph too big, reduce network
all_simple_paths(g1,from=mets.mapping[node1[1],]$mets,
                 to=mets.mapping[node2[1],]$mets,mode='all')
