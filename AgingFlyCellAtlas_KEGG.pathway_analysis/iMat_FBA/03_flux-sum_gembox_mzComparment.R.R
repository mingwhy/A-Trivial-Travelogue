...
cell.type_per.age=as.data.frame(res$df.info)
head(cell.type_per.age)
dim(cell.type_per.age) #108 x2 

datf=res$df.flux.vec
dim(datf) #11898   108
ncol(datf)==nrow(cell.type_per.age)

length(abs(ifly$S) %*% abs(datf [,1])) #mz length
mz.fluxsum=abs(ifly$S) %*% abs(datf)
dim(mz.fluxsum) #8135   108 cell.type_per.age

cell.type=cell.type_per.age$cell.type
age=cell.type_per.age$age
